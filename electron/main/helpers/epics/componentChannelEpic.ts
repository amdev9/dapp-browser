...
// const openChannel = (uuid: string) => ({ type: constants.OPEN_CHANNEL, payload: { uuid: uuid } });
...
const componentChannelEpic: Epic<ChannelAction> = action$ => action$.pipe(
  ofType(constants.INTENT_CHANNEL_DATA_PASS),
  
  mapTo({ type: constants.ACCEPT_CHANNEL_DATA_PASS }),   
