...
const cleanRepo = async (repoPath: string) => {
  ...
  fs.access(lockPath, fs.constants.F_OK, (err) => {
    if (err) { console.warn(`unable to access file here: ${lockPath}`); } // throw err;
    fs.unlink(lockPath, (err) => {
      if (err) { console.warn(`unable to delete file here: ${lockPath}`); } // throw err;
    });
  });
};

export const getReadyIpfsInstance = async (options: IPFS.Options = { repo: path.join(__dirname, 'ipfs', 'repo') }): Promise<any> => {
  const ipfs = new IPFS({
    ...remoteConfig,
